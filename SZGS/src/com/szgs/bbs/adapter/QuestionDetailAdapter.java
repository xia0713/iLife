...
public class QuestionDetailAdapter extends BaseAdapter {
	...
	public QuestionDetailAdapter(Context context,
			ArrayList<QuestionListResponse.Question> list, int from) {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (from == 4) {
			...
			if (list.get(position).question != null) {
				...
				if (list.get(position).answerCount < hotAnswerCount) {
					...
					if (list.get(position).answerCount == 0) {
						...
						holder.question_sum2
								.setText(LggsUtils.caculateLikeNum(list
										.get(position).answerCount) + "人回答");
					}
				...
				ImageLoader.getInstance().displayImage(
						list.get(position).askBy.avatar,
						holder.img_question_icon, options);
			...
			questioncontent += list.get(position).title;
			// holder.tv_question_content.append(list.get(position).title);
			if (list.get(position).askBy.id.equals(CacheUtils
					.getUserName(context))) {
			...
			if (list.get(position).answerCount == 0) {
				...
				holder.question_sum.setText(list.get(position).answerCount
						+ "人回答");
			}
			if (from == 3) {
				holder.hot_img.setBackgroundResource(R.drawable.list_credit);
				holder.tv_question_score.setText(list.get(position).rewardScore
						+ "分");
				holder.hot_img.setVisibility(View.GONE);
				holder.tv_question_score.setVisibility(View.GONE);
				holder.ll_home_hot.setVisibility(View.GONE);
			...
			holder.ll_question_tag.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					LggsUtils.StartIntent(context, CategoryActivity.class,
							bundle);
				}
			});
			// 去回答该问题
			holder.tv_question_toanswer
